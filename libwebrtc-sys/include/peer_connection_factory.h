...
public:
    ArcasPeerConnectionFactory(rtc::scoped_refptr<ArcasAPIInternal> internal_api, rtc::scoped_refptr<webrtc::PeerConnectionFactoryInterface> api);

    /**
     * @pre observer is not null
     */
    std::shared_ptr<ArcasPeerConnection> create_peer_connection(std::unique_ptr<webrtc::PeerConnectionInterface::RTCConfiguration> config,
                                                                ArcasPeerConnectionObserver* observer) const
    {
        ...
        {
            RTC_LOG(LS_ERROR) << "The peer connection observer passed into create a peer connection should not "
