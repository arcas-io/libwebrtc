...
std::unique_ptr<webrtc::SdpVideoFormat> create_sdp_video_format(ArcasSdpVideoFormatInit init)
{
    ...
    {
        cxx_parameters.insert(
            std::make_pair(std::string(item.key.c_str()), std::string(item.value.c_str())));
    }
...
create_sdp_video_format_list(ArcasSdpVideoFormatVecInit list)
{
    ...
    for (auto rust_sdp_video_format : list.list)
    {
        ...
        output->push_back(webrtc::SdpVideoFormat(std::string(rust_sdp_video_format.name.c_str()),
                                                 cxx_parameters));
    }
