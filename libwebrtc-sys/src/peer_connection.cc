...
void ArcasPeerConnection::set_local_description(rust::Box<ArcasRustSetSessionDescriptionObserver> observer,
...
void ArcasPeerConnection::set_remote_description(rust::Box<ArcasRustSetSessionDescriptionObserver> observer,
                                                 std::unique_ptr<ArcasSessionDescription> sdp) const
{
    auto ref_counted = rtc::make_ref_counted<ArcasSetDescriptionObserver>(std::move(observer));
...
std::unique_ptr<ArcasRTPVideoTransceiver> ArcasPeerConnection::add_video_transceiver_with_track(std::unique_ptr<ArcasVideoTrack> track,
                                                                                                ArcasTransceiverInit init) const
{
    webrtc::RtpTransceiverInit transceiver_init;
    transceiver_init.direction = init.direction;

    for (auto stream_id : init.stream_ids) { transceiver_init.stream_ids.push_back(std::string(stream_id.c_str())); }
...
std::unique_ptr<ArcasRTPAudioTransceiver> ArcasPeerConnection::add_audio_transceiver_with_track(std::unique_ptr<ArcasAudioTrack> track,
                                                                                                ArcasTransceiverInit init) const
{
    webrtc::RtpTransceiverInit transceiver_init;
    transceiver_init.direction = init.direction;
...
void ArcasPeerConnection::add_ice_candidate(std::unique_ptr<ArcasICECandidate> candidate) const
{
    ...
    auto add_candidate =
        std::unique_ptr<webrtc::IceCandidateInterface>(webrtc::CreateIceCandidate(jsep->sdp_mid(), jsep->sdp_mline_index(), sdp_out, &error));
...
std::unique_ptr<ArcasDataChannel> ArcasPeerConnection::create_data_channel(rust::String label, const ArcasDataChannelInit& init_rust) const
{
