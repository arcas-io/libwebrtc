...
void ArcasPeerConnectionFactoryConfig::set_video_encoder_factory(
    rust::Box<ArcasRustVideoEncoderFactory> api)
{
    video_encoder_factory = create_arcas_video_encoder_factory(std::move(api));
}

void ArcasPeerConnectionFactoryConfig::set_video_decoder_factory(
    rust::Box<ArcasRustVideoDecoderFactory> api)
{
    video_decoder_factory = create_arcas_video_decoder_factory(std::move(api));
}

void ArcasPeerConnectionFactoryConfig::set_audio_encoder_factory(
    rust::Box<ArcasRustAudioEncoderFactory> api)
{
    audio_encoder_factory = absl::optional<rtc::scoped_refptr<ArcasAudioEncoderFactory>>(
        create_audio_encoder_factory(std::move(api)));
}