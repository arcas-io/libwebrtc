...
void ArcasReactiveVideoEncoderWrapper::on_loss_notification(ArcasVideoEncoderLossNotification loss) const
{
    ...
    {
        notification.dependencies_of_last_received_decodable.emplace(loss.dependencies_of_last_received_decodable[0]);
    }
...
ArcasVideoEncoderInfo ArcasReactiveVideoEncoderWrapper::get_encoder_info() const
{
    ...
    for (auto i = 0; i < webrtc::kMaxTemporalStreams; i++)
    {
        ...
        for (auto j = 0; j < webrtc::kMaxSpatialLayers; j++) { rust_fps_allocation.allocation.push_back(info.fps_allocation[i][j]); }
