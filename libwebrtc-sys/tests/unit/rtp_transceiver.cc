...
namespace
{
std::vector<std::pair<ArcasRustRTCStatsCollectorCallback*, rtc::scoped_refptr<const webrtc::RTCStatsReport>>> stats_delivered;
}
...
TEST_CASE("get_stats() : enqueues a callback for sender and receiver", "[ArcasRTPVideoTransceiver]")
{
    ...
    test_webrtc_peer_connection pc;
    rtc::scoped_refptr<webrtc::RtpTransceiverInterface> api{new webrtc::test_webrtc_peer_rtp_transceiver{}};
    ArcasRTPVideoTransceiver tsrv{pc, api};
    ArcasRustRTCStatsCollectorCallback cb;
    //using decltype here is abnormal, but I was using CTAD and this is what CTAD does
    tsrv.get_stats(rust::Box<decltype(cb)>{cb});
    CHECK(pc.senders_awaiting_stats_callbacks_.size() == 1);
    CHECK(pc.recvers_awaiting_stats_callbacks_.size() == 1);
}

void ArcasRTCStatsCollectorCallback::OnStatsDelivered(rtc::scoped_refptr<const webrtc::RTCStatsReport> const& report)
{
...
template<>
ArcasRustRTCStatsCollectorCallback* rust::cxxbridge1::Box<ArcasRustRTCStatsCollectorCallback>::allocation::alloc() noexcept
{
    return nullptr;
}
template<>
void rust::cxxbridge1::Box<ArcasRustRTCStatsCollectorCallback>::allocation::dealloc(ArcasRustRTCStatsCollectorCallback*) noexcept
{
}
template<>
